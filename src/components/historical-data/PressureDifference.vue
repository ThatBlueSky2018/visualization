<template>
  <div id="pressure_difference">
    <div id="range">
      ...
      <div id="pump_id">
        ...
        <select v-model="selectedPump" id="select_pump">
          <option value="pressure_difference_1">泵站1</option>
          <option value="pressure_difference_2">泵站2</option>
          <option value="pressure_difference_3">泵站3</option>
          <option value="pressure_difference_4">泵站4</option>
        </select>
      </div>
      <div id="submit">
    ...
    <div id="pressure_difference_data"></div>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      selectedWeek: null,
      selectedPump: "pressure_difference_1",
      weekDateRange: {
        startDate: null,
        endDate: null,
      },
      type: "天",
      request_tail: "pressure_difference_day/", //请求路径尾部
      chartData: {
        date: [],
        pressure_diff: [],
      },
  ...
  methods: {
    handleTypeChange(val) {
      this.type = val;
      if (val === "天") {
        this.request_tail = "pressure_difference_day/";
        this.selectedWeek = null; // 选择"天"时，清空周选择器的值
      } else if (val === "周") {
        this.request_tail = "pressure_difference_week/";
    ...
    fetchData() {
      ...
      axios
        .get("http://127.0.0.1:8000/api/data/" + this.request_tail, {
          params: {
            time: this.type === "天" ? this.formattedDate : this.weekDateRange,
            pump_id: this.selectedPump,
          },
        })
        .then((response) => {
          this.chartData.date = response.data.datetime;
          this.chartData.pressure_diff = response.data.pressure_diff;

          var chartDom = document.getElementById("pressure_difference_data");
          ...
          option = {
            ...
            series: [
              {
                data: this.chartData.pressure_diff,
                type: "line",
                smooth: true,
              },
            ],
